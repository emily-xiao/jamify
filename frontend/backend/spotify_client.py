...
class SpotifyClient:
    ...
    def get_valence(self, tracks):
        valence_list = []
        for track in tracks:
            url = f"https://api.spotify.com/v1/audio-features?ids={track.id}"
            response = self._place_get_api_request(url)
            response_json = response.json()
            valence = response_json["audio_features"][0]["valence"]
            valence_list.append(valence)
        return valence_list
    
    def nearest_neighbours(self, input, tracks, valence):
        target = input/5
        keys = tracks
        values = [abs(val-target) for val in valence]
        markdict = {keys[i]:values[i] for i in range(len(keys))}
